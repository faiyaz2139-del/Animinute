...
export default function AMHome() {
  ...
  const [projectPopup, setProjectPopup] = useState({ open: false, business: null, projects: [] });
  ...
  const fetchProjects = async (business) => {
    try {
      const res = await axios.get(`${AM_API_URL}/projects`, { params: { business_id: business.id } });
      setProjectPopup({ open: true, business, projects: res.data });
    } catch (err) {
      console.error(err);
    }
  };
      ...
      <div data-testid="am-home">
        <div className="am-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <BlueButton onClick={() => navigate('/anyminute/add-business')}>Add Business</BlueButton>
          </div>
      ...
      {projectPopup.open && (
        <div style={{
          ...
          <div className="am-card" style={{ width: '90%', maxWidth: '700px', maxHeight: '80vh', overflow: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3>Projects - {projectPopup.business?.company_name}</h3>
              <button onClick={() => setProjectPopup({ open: false, business: null, projects: [] })} style={{ border: 'none', background: 'none', fontSize: '20px', cursor: 'pointer' }}>×</button>
            </div>
            <Table
              columns={[
